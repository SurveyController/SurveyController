...
def call_free_ai_api(
    ...
    for attempt in range(2):
        ...
        except FreeAITimeoutError:
            raise
        except Exception as exc:
            last_exc = exc
            if attempt == 0:
                logger.warning("免费 AI 调用失败，尝试轮换设备身份后重试：%s", exc)
                try:
                    reset_device_identity()
                    activate_trial()
                except Exception as retry_exc:
                    logger.warning("设备身份轮换失败：%s", retry_exc)
            else:
                raise
    raise last_exc  # type: ignore[misc]


def _call_free_ai_api_once(
    question: str,
    question_type: str,
    blank_count: Optional[int],
    system_prompt: str = "",
