...
class SubmissionService:
    ...
    def _detect_validation_error(self, driver: BrowserDriver) -> Optional[str]:
        ...
        try:
            ...
            result = detect_post_submit_errors(driver)
            if result is None:
                return None
            q_ids = result.unanswered_question_ids
            if q_ids:
                return f"{result.error_text} | 未答题: {', '.join(q_ids)}"
        ...
        *,
        ...
        if self._is_wjx_provider():
            ...
            if not stop_signal.is_set():
                ...
                if validation_error:
                    logging.warning("提交后检测到验证错误：%s", validation_error)
                    return SubmissionOutcome(
                        status="failure",
