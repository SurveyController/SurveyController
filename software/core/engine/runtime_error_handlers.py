...
<<<<<<< HEAD
    ...
    if is_free_ai_runtime_error(exc):
        ...
        stopped = stop_policy.record_failure(
            ...
            status_text="免费AI失败",
            log_message=(
                f"免费AI调用失败，本轮按失败处理；连续达到 {FREE_AI_ERROR_FAIL_THRESHOLD} 次才停止：{exc}"
            ),
            threshold_override=FREE_AI_ERROR_FAIL_THRESHOLD,
            terminal_stop_category="free_ai_unstable",
            force_stop_when_threshold_reached=True,
            consume_reverse_fill_attempt=False,
        )
        if stopped:
            logging.error("免费 AI 连续失败达到阈值，任务停止：%s", exc, exc_info=True)
        return bool(stopped)
