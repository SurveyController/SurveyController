...
def _normalize_user_agent_ratios(raw: Any) -> Dict[str, int]:
    if not isinstance(raw, dict):
        return dict(_DEFAULT_RANDOM_UA_RATIOS)
    normalized: Dict[str, int] = {}
    for key, value in raw.items():
        device = str(key or "").strip().lower()
        if device not in _USER_AGENT_DEVICE_TO_PRESET_KEYS:
            return dict(_DEFAULT_RANDOM_UA_RATIOS)
        weight = _coerce_int(value, -1)
        if weight < 0:
            return dict(_DEFAULT_RANDOM_UA_RATIOS)
        normalized[device] = weight
    if set(normalized) != set(_USER_AGENT_DEVICE_TO_PRESET_KEYS):
        return dict(_DEFAULT_RANDOM_UA_RATIOS)
...
def _select_user_agent_from_ratios(ratios: Dict[str, int]) -> Tuple[Optional[str], Optional[str]]:
    ...
    normalized = _normalize_user_agent_ratios(ratios)
    devices = list(_USER_AGENT_DEVICE_TO_PRESET_KEYS)
    weights = [max(0, int(normalized.get(device, 0))) for device in devices]
    if sum(weights) <= 0:
        return None, None
    ...
    key = random.choice(list(ua_keys))
...
def serialize_runtime_config(config: RuntimeConfig) -> Dict[str, Any]:
    ...
    payload.pop("random_ua_keys", None)
