...
() => {
  ...
  Array.from(document.querySelectorAll('.answer-page .question')).forEach((root) => {
    ...
    if (!root.querySelector(interactiveSelector)) {
      const hasRealInput = Array.from(root.querySelectorAll(inputSelector)).some((node) => {
...
(el) => {
  ...
  if (!el.querySelector(interactiveSelector)) {
    const realInputs = Array.from(el.querySelectorAll(
...
def _text_inputs(root: Any) -> List[Any]:
    ...
    for inp in all_inputs:
        try:
            tag = str(inp.evaluate("el => el.tagName") or "").upper()
            in_rich = bool(inp.evaluate("el => !!el.closest('.rich-text-container')"))
            if tag == "TEXTAREA" and in_rich:
                continue
            in_other = bool(inp.evaluate(
                "el => { const row = el.closest('.choice-row, .choice, [class*=\"other\"]'); "
                "if (!row) return false; "
                "const t = row.querySelector('.choice-text'); "
                "return t && /其他|请说明|other/i.test(t.innerText || ''); }"
            ))
            if in_other:
                continue
        except Exception:
...
def _navigation_action(page: Any) -> Optional[str]:
    return _detect_navigation_action_common(
        page,
        locator_fn=page.locator,
        count_fn=_safe_count,
        visible_fn=_locator_is_visible,
        text_fn=_text_fn,
        attr_fn=_attr_fn,
    )
