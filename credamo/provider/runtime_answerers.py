...
(el) => {
  ...
  return Array.from(inputs).filter((node) => {
    if (!inputVisible(node)) return false;
    // Exclude textareas inside rich-text containers (description boxes)
    if (node.tagName === 'TEXTAREA' && node.closest('.rich-text-container')) return false;
    // Exclude inputs inside "其他(请说明)" option rows
    const choiceRow = node.closest('.choice-row, .choice, [class*="other"]');
    if (choiceRow) {
      const choiceText = choiceRow.querySelector('.choice-text');
      if (choiceText && /其他|请说明|other/i.test(choiceText.innerText || '')) return false;
    }
    return true;
  }).map((node, i) => i);
}
"""
    try:
        visible_indices = page.evaluate(script, root) or []
    except Exception:
        return _text_inputs(root)
    all_inputs = _text_inputs(root)
...
def _answer_text(
    ...
    *,
    ...
    inputs = _visible_text_inputs(page, root)
    if not inputs:
        return False
    values = text_config if isinstance(text_config, list) and text_config else [DEFAULT_FILL_TEXT]

    # Try AI generation for single text questions
    if ai_enabled and len(inputs) == 1:
        try:
            from software.core.ai.runtime import AIRuntimeError, generate_ai_answer
            title = str(question_title or "").strip()
            if title:
                ai_answer = generate_ai_answer(title, question_type="fill_blank", blank_count=1)
                if isinstance(ai_answer, list):
                    values = [str(ai_answer[0]).strip()] if ai_answer else values
                else:
                    answer_text = str(ai_answer or "").strip()
                    if answer_text:
                        values = [answer_text]
                logging.info("Credamo 第%s题 AI 填空成功：title=%s answer=%s", question_num, title, values[0])
        except Exception as exc:
            logging.warning("Credamo 第%s题 AI 填空失败，回退到配置答案：%s", question_num, exc)

    # Try AI generation for multi-text questions
    if ai_enabled and len(inputs) > 1:
        try:
            from software.core.ai.runtime import AIRuntimeError, generate_ai_answer
            title = str(question_title or "").strip()
            if title:
                ai_result = generate_ai_answer(title, question_type="multi_fill_blank", blank_count=len(inputs))
                if isinstance(ai_result, list) and ai_result:
                    values = [str(item).strip() or DEFAULT_FILL_TEXT for item in ai_result]
                    logging.info("Credamo 第%s题 AI 多项填空成功：title=%s answers=%s", question_num, title, values)
        except Exception as exc:
...
def _answer_order(page: Any, root: Any, weights: Any = None) -> bool:
    ...
    if isinstance(weights, (list, tuple)) and len(weights) == count and any(w > 0 for w in weights):
        ...
        order: list[int] = []
        temp_weights = [float(weights[i]) for i in remaining]
        while remaining:
            ...
            for i, w in enumerate(temp_weights):
                ...
                if r <= cumulative:
                    picked_idx = i
                    break
            order.append(remaining.pop(picked_idx))
            temp_weights.pop(picked_idx)
    else:
