...
<<<<<<< HEAD
    ...
    if (!root.querySelector(interactiveSelector)) {
      const realInputs = Array.from(root.querySelectorAll(
...
def _parser_attr_fn(locator: Any) -> str:
    return _normalize_text(locator.get_attribute("value"))
...
def _detect_navigation_action(page: Any) -> Optional[str]:
    return _detect_navigation_action_common(
        ...
        count_fn=_locator_count,
        visible_fn=_locator_is_visible,
        text_fn=_parser_text_fn,
