...
class _FakeQuestionPage:
    ...
    def __init__(self) -> None:
        self._entries = [QuestionEntry(question_type='single', probabilities=[100.0], texts=['A'], option_count=1, question_num=1)]
        self.questions_info = [SurveyQuestionMeta(num=1, title='题目一', type_code='3', provider_question_id='q1')]
...
class ConfigSnapshotUsageTests:
    ...
    def test_run_actions_start_uses_shared_snapshot_helper(self) -> None:
        ...
        with patch('software.ui.pages.workbench.dashboard.parts.run_actions.build_runtime_config_snapshot', return_value=snapshot) as snapshot_helper:
            host._on_start_clicked()
        assert host.controller.started_config is snapshot
        assert snapshot_helper.call_count == 1
        assert snapshot_helper.call_args.kwargs['question_entries'] == host.question_page.get_entries()
        assert snapshot_helper.call_args.kwargs['questions_info'] == host.question_page.questions_info
    ...
    def test_config_save_uses_shared_snapshot_helper(self) -> None:
        ...
        with patch('software.ui.pages.workbench.dashboard.parts.config_io.build_runtime_config_snapshot', return_value=snapshot) as snapshot_helper, patch('software.ui.pages.workbench.dashboard.parts.config_io.QFileDialog.getSaveFileName', return_value=('D:/demo.json', 'JSON 文件 (*.json)')):
            host._on_save_config()
        assert host.controller.config is snapshot
        assert host.controller.saved_path == 'D:/demo.json'
        assert snapshot_helper.call_count == 1
        assert snapshot_helper.call_args.kwargs['question_entries'] == host.question_page.get_entries()
        assert snapshot_helper.call_args.kwargs['questions_info'] == host.question_page.questions_info
    ...
    def test_lifecycle_snapshot_collection_uses_shared_snapshot_helper(self) -> None:
        ...
        with patch('software.ui.shell.main_window_parts.lifecycle.build_runtime_config_snapshot', return_value=snapshot) as snapshot_helper:
