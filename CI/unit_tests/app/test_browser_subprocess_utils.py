...
class BrowserSubprocessUtilsTests:
    ...
    def test_list_browser_pids_uses_local_text_decode_settings(self) -> None:
        ...
        with patch('software.network.browser.transient.subprocess.run', return_value=SimpleNamespace(returncode=0, stdout=tasklist_output, stderr='')) as run_mock, patch('software.network.browser.transient.build_local_text_subprocess_kwargs', return_value={'text': True, 'encoding': 'cp936', 'errors': 'replace'}):
    ...
    def test_force_terminate_browser_process_tree_uses_local_text_decode_settings(self) -> None:
        ...
        with patch('software.network.browser.session.subprocess.run', return_value=SimpleNamespace(returncode=0, stdout='', stderr='')) as run_mock, patch('software.network.browser.session.build_local_text_subprocess_kwargs', return_value={'text': True, 'encoding': 'cp936', 'errors': 'replace'}):
