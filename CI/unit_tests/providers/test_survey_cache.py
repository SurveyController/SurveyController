...
class SurveyCacheTests:
    ...
    def test_same_fingerprint_reuses_cached_definition(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert os.path.isdir(os.path.join(temp_dir, 'configs', 'survey_cache'))
    ...
    def test_clear_survey_parse_cache_blocks_late_background_rewrite(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                survey_cache.clear_survey_parse_cache()
                survey_cache._write_cached_definition(cache_path, build_survey_definition('wjx', '标题', [{'num': 1, 'title': 'Q1', 'type_code': '3'}]), 'fingerprint', expected_epoch=epoch_before_clear)
                wrote_attempt.set()
            finally:
                survey_cache.get_runtime_directory = original_runtime_directory
