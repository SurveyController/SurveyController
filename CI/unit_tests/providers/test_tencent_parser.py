...
class TencentParserTests:
    ...
    @pytest.mark.asyncio
    async def test_parse_qq_survey_returns_http_result(self, patch_attrs) -> None:
        patch_attrs(
            (
                qq_parser,
    ...
    @pytest.mark.asyncio
    async def test_parse_qq_survey_rejects_login_url_and_login_required_http_error(self, patch_attrs) -> None:
        with pytest.raises(RuntimeError, match="需要登录"):
            await qq_parser.parse_qq_survey("https://wj.qq.com/r/login.html")

        patch_attrs((qq_parser, "_fetch_qq_survey_via_http", AsyncMock(side_effect=RuntimeError("need login"))))

        with pytest.raises(RuntimeError, match="需要登录"):
            await qq_parser.parse_qq_survey("https://wj.qq.com/s2/123/hash/")

    @pytest.mark.asyncio
    async def test_parse_qq_survey_http_error_is_terminal(self, patch_attrs) -> None:
        patch_attrs((qq_parser, "_fetch_qq_survey_via_http", AsyncMock(side_effect=RuntimeError("http failed"))))
