...
class ProviderRegistryTests(unittest.IsolatedAsyncioTestCase):
    async def test_parse_survey_routes_detected_provider_through_cache_loader(self) -> None:
    ...
    async def test_fill_survey_uses_provider_run_context_and_selected_adapter(self) -> None:
    ...
    async def test_handle_submission_verification_detected_uses_ctx_config_provider(self) -> None:
    ...
    async def test_wait_for_submission_verification_routes_timeout_and_stop_signal(self) -> None:
        adapter = registry._PROVIDER_REGISTRY[SURVEY_PROVIDER_QQ]
        driver = object()
