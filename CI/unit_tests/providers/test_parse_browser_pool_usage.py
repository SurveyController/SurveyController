...
class ParseBrowserPoolUsageTests(unittest.TestCase):
    def test_wjx_fallback_uses_parse_browser_pool(self) -> None:
        ...
        with (
            patch("wjx.provider.parser.http_client.get", side_effect=RuntimeError("http failed")),
            patch("wjx.provider.parser.acquire_parse_browser_session", fake_pool),
            patch("wjx.provider.parser.parse_survey_questions_from_html", return_value=[{"num": 1, "title": "Q1", "type_code": "3"}]),
            patch("wjx.provider.parser.extract_survey_title_from_html", return_value="问卷标题"),
        ):
            info, title = wjx_parser.parse_wjx_survey("https://www.wjx.cn/vm/demo.aspx")

        self.assertTrue(used_pool["value"])
        self.assertEqual(title, "问卷标题")
    ...
    def test_qq_fallback_uses_parse_browser_pool(self) -> None:
        ...
        payload = {
            ...
            "title": "腾讯问卷标题",
            "metaPayload": {"code": "OK", "data": {"title": "腾讯问卷标题"}},
            "payload": {"code": "OK", "data": {"questions": [{"id": "1", "type": "radio", "title": "Q1", "options": [{"text": "A"}]}]}},
        }
        ...
        with (
            patch("tencent.provider.parser._fetch_qq_survey_via_http", side_effect=RuntimeError("http failed")),
            patch("tencent.provider.parser.acquire_parse_browser_session", fake_pool),
        ):
            info, title = qq_parser.parse_qq_survey("https://wj.qq.com/s2/123/hash/")

        self.assertTrue(used_pool["value"])
        self.assertEqual(title, "腾讯问卷标题")
        self.assertEqual(len(info), 1)
    ...
    def test_credamo_parser_uses_parse_browser_pool(self) -> None:
        ...
        with (
            patch("credamo.provider.parser.acquire_parse_browser_session", fake_pool),
            patch("credamo.provider.parser._retry_initial_question_load_if_needed", return_value=[object()]),
            patch("credamo.provider.parser._collect_current_page_until_stable", return_value=([{"question_num": "Q1"}], [{"question_num": "Q1", "title": "Q1", "question_kind": "single", "provider_type": "single", "option_texts": ["A"], "text_inputs": 0, "page": 1, "question_id": "q1"}])),
            patch("credamo.provider.parser._detect_navigation_action", return_value="submit"),
        ):
            info, title = credamo_parser.parse_credamo_survey("https://www.credamo.com/answer.html#/s/demo")

        self.assertTrue(used_pool["value"])
        self.assertEqual(title, "Credamo 标题")
        self.assertEqual(len(info), 1)
