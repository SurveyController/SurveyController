...
from software.network.proxy.session.normalize import (
...
class RandomIPSessionNormalizeTests(unittest.TestCase):
    def test_format_quota_value_normalizes_decimal_and_invalid_input(self) -> None:
        self.assertEqual(format_quota_value("2.5000"), "2.5")
        self.assertEqual(format_quota_value("3.0"), "3")
        self.assertEqual(format_quota_value("-1"), "0")
        self.assertEqual(format_quota_value("bad"), "0")
    ...
    def test_normalize_quota_state_prefers_used_quota_when_provided(self) -> None:
        ...
        self.assertEqual((remaining, total, used), (0.0, 7.0, 7.0))
    ...
    def test_normalize_quota_state_backfills_used_quota_from_remaining(self) -> None:
        ...
        self.assertEqual((remaining, total, used), (2.0, 5.0, 3.0))

    def test_require_valid_user_id_rejects_non_positive_value(self) -> None:
        with self.assertRaises(RandomIPAuthError):
    ...
    def test_resolve_quota_from_payload_accepts_complete_payload(self) -> None:
        remaining, total, used, known = _resolve_quota_from_payload(
            {
                "remaining_quota": "2",
                "total_quota": "5",
                "used_quota": "3",
        ...
        self.assertEqual((remaining, total, used, known), (2.0, 5.0, 3.0, True))
    ...
    def test_resolve_quota_from_payload_reuses_fallback_for_partial_payload(self) -> None:
        ...
        self.assertEqual((remaining, total, used, known), (6.0, 10.0, 4.0, True))
    ...
    def test_resolve_quota_from_payload_marks_missing_numbers_as_untrusted(self) -> None:
        remaining, total, used, known = _resolve_quota_from_payload(
            ...
            fallback_session=RandomIPSession(),
            log_context="test",
        )

        self.assertEqual((remaining, total, used, known), (0.0, 0.0, 0.0, False))
