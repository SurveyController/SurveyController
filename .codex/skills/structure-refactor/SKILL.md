...
description: 面向 SurveyController 仓库的目录结构重构与模块拆分技能。用于整理 software/、wjx/、tencent/、credamo/、go/、CI/unit_tests/ 等真实目录，处理文件移动、命名调整、导入修复、测试路径更新、资源路径修复、CI 检查联动，以及当前 refactor/go 分支的 Go+Wails 迁移目录边界。适合小步重构、保持行为不变、清理屎山边界，不适合纯样式改名或无关大洗牌。
---
...
目标是让 `software/`、平台适配目录、`go/`、测试目录和资源目录的职责更清楚。
...
- 需要把 Python/PySide 模块逐步迁移到 `go/` 或后续 Wails 模块。
...
2. 先画出这次改动涉及的目录。
   ...
   - `go/` 放 Go 原型、Go 核心库和后续 Wails 应用模块。
...
- 当前 `refactor/go` 分支目标是迁到 Go+Wails；Python/PySide 仍是可运行基线。
- `go/` 是 Go 迁移目录。纯核心库和 Wails 应用壳要分开，不要把 UI 壳塞进核心库。
...
- Go+Wails 迁移也保持纯 HTTP 提交链路。Wails WebView 只做桌面 UI，不恢复浏览器自动化兜底。
